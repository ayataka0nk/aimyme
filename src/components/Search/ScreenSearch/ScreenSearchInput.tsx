...
export const ScreenSearchInput = forwardRef<HTMLInputElement, Props>(
  ({ layer, className, onClearClick, onBackClick, ...props }, ref) => {
    const layerStyle = getLayerStyle(layer)
    return (
      <div className={`${className}`}>
        <div
          className={`cursor-pointer relative h-14 flex items-center hover:after:absolute hover:after:inset-0 hover:after:bg-on-surface hover:after:opacity-8 after:pointer-events-none`}
        >
          <input
            ref={ref}
...
ScreenSearchInput.displayName = 'ScreenSearchInput'
